...
<p align="center">
  ...
  <img alt="Версия" src="https://img.shields.io/badge/version-v0.1.1-8A2BE2">
...
В архиве ровно один исполняемый файл — больше ничего устанавливать не нужно.
Python не требуется.
...
Результат: `dist\VoxVault.exe` и `dist\VoxVault-<версия>-win64.zip` — в zip
лежит **ровно один файл**, его и прикрепляют к релизу на GitHub. Никаких
папок `_internal` и россыпи DLL: скачал, распаковал, запустил.

Релиз собирается с `--onefile`: программа сама распаковывает свои библиотеки
во временный каталог Windows при каждом запуске. Замеренный старт — около
2,0 с против 1,0 с у сборки-папки.

### Сборка папкой вместо одного файла

`onedir` тоже доступен: он быстрее стартует и реже вызывает антивирус,
но занимает ~1000 файлов на диске:

```powershell
python packaging\build.py --clean --zip
```

| | папка (`onedir`) | один файл (`onefile`, для релизов) |
|---|---|---|
| запуск | ~1,0 с | ~2,0 с |
...
Для быстрого ежедневного запуска лучше сборка-папка, но в релиз мы кладём один
файл — чтобы пользователю было скачать и запустить ровно одно.

### Установщик вместо zip

В архиве релиза уже ровно один исполняемый файл. Если нужен привычный
«один setup.exe» с ярлыком в меню «Пуск» — это отдельная задача
...
Релиз публикуется тегом: пуш `v0.1.0` → GitHub Actions собирает exe, прогоняет
...
- Загрузка идёт во временный `.part` с тремя попытками: оборванный файл не
  ...
  `conf/mfcc.conf`, `conf/model.conf`, `graph/phones/word_boundary.int`) и
  хотя бы одного файла графа `graph/*.fst`. Недокачанная модель удаляется.
  Две ловушки, обе уже стоили 1,8 ГБ трафика:
  - имя файла графа **везде разное**: у старых моделей это `Gr.fst` + `HCLr.fst`,
    у новых (`ru-0.42`, `en-us-0.22`, `de-0.21`, `uk-v3`, `cn`, `ja`, `el-gr`)
    — один `HCLG.fst`. Требование `Gr.fst` удаляет исправную модель как
    «неполную»;
  - `graph/words.txt` в части официальных архивов VOSK **отсутствует**,
    требовать его тоже нельзя.
